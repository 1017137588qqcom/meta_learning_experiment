...
def train(epoch, dataloader, net, optimizer, criterion, vali=True):
    ...
    for batch_id, (inputs, targets) in enumerate(dataloader):
        ...
        targets_onehot.scatter_(1, targets.long().unsqueeze(-1), 1).float()
        y_onehot = targets_onehot

        loss = criterion(F.softmax(outputs), y_onehot) # .long()
        loss.backward()
        optimizer.step()
...
def test(epoch, dataloader, net, criterion, vali=True):
    ...
    with torch.no_grad():
        for batch_id, (inputs, targets) in enumerate(dataloader):
            ...
            loss = criterion(F.softmax(outputs), y_onehot)
    ...
    if vali is True:
        ...
        if acc >= best_acc:
            ...
            if not os.path.isdir('best_acc'):
                os.mkdir('best_acc')
            torch.save(state1, './best_acc/best_MSE''.t7')
            best_acc = acc
            best_acc1 = open('./test_dir/MSE.txt', 'w')
    ...
    for avg in range(10):
        ...
        for epoch in range(start_epoch, start_epoch+args.max_epoch):
            ...
            if not os.path.isdir('finial'):
                os.mkdir('finial')
            torch.save(state3, './finial/MSE'+'.t7')
            ...
            plt.plot(np.arange(0, len(train_array0)), train_array0, '-')
            plt.subplot(2, 2, 2)
            plt.xlabel('step')
            plt.ylabel('train acc')
            plt.plot(np.arange(0, len(train_array1)), train_array1, '-')
            plt.subplot(2, 2, 3)
            plt.xlabel('step')
            plt.ylabel('test loss')
            plt.plot(np.arange(0, len(test_array0)), test_array0, '-')
            plt.subplot(2, 2, 4)
            plt.xlabel('step')
            plt.ylabel('test acc')
            plt.plot(np.arange(0, len(test_array1)), test_array1, '-')
            plt.tight_layout()
            plt.savefig('./test_dir/MSE'+str(avg)+'.jpg')
            plt.close()
        ...
        file.to_csv('./test_dir/MSE'+str(avg)+'.csv')