...
import argparse
import numpy as np
from utils import progress_bar, set_parameters
from model import ResNet18, LeNet_FashionMNIST, LeNet_Cifar10, VGG16
...
def model_prepare(work):
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.5, patience=5, verbose=True, threshold=1e-8, threshold_mode='rel')

    criterion = MetaOptimizer(to_device=device, is_binary=False)  #
    set_parameters(criterion.rnn, pop['DNA'][24], bias=True)
    ...
    if vali is True:
        ...
        if acc >= best_acc:
            ...
            if not os.path.isdir('best_acc'):
                os.mkdir('best_acc')
            torch.save(state1, './best_acc/best_MLN''.t7')
            best_acc = acc
            best_acc1 = open('./test_dir/MLN.txt', 'w')
    ...
    for avg in range(10):
        ...
        for epoch in range(start_epoch, start_epoch+args.max_epoch):
            ...
            if not os.path.isdir('finial'):
                os.mkdir('finial')
            torch.save(state3, './finial/MLN'+'.t7')
            ...
            plt.tight_layout()
            plt.savefig('./test_dir/MLN'+str(avg)+'.jpg')
            plt.close()
        ...
        file.to_csv('./test_dir/MLN'+str(avg)+'.csv')